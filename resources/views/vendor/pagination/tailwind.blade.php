@php
    ...
    // Build visible page numbers centered around current page
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-0.5">
                ...
                @foreach ($visiblePages as $page)
                    ...
                    @else
                        <a href="{{ $paginator->url($page) }}" class="inline-flex items-center justify-center min-w-[2rem] h-8 rounded-md text-sm font-semibold text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700 px-2">
                            {{ $page }}
                        </a>
                    @endif
                @endforeach
            ...
            {{-- Go to page input (only when more than maxVisible pages) --}}
            @if ($lastPage > $maxVisible)
                <form method="GET" action="{{ $paginator->url($currentPage) }}" class="flex items-center gap-1.5">
                    ...
                    <input
                        ...
                        max="{{ $lastPage }}"
                        value="{{ $currentPage }}"
                        class="w-14 px-2 py-1 text-xs text-center border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500"
                    >
                    <span class="text-xs text-gray-500 dark:text-gray-400">of {{ $lastPage }}</span>
                    </button>
                </form>
            @endif
        </div>
